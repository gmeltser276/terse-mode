#!/usr/bin/env node
// terse-mode — shared configuration resolver
//
// Resolution order for default mode:
//   1. TERSE_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/terse-mode/config.json (any platform, if set)
//      - ~/.config/terse-mode/config.json (macOS / Linux fallback)
//      - %APPDATA%\terse-mode\config.json (Windows fallback)
//   3. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'terse-mode');
  }
  ...
  return path.join(os.homedir(), '.config', 'terse-mode');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.TERSE_DEFAULT_MODE;
...
// flag path (~/.claude/.terse-active) with a symlink to clobber other files.
//
...
//
// Set TERSE_DEBUG=1 to emit stderr diagnostics when flag writes are refused.
//
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (!realStat.isDirectory()) {
          if (debug) process.stderr.write(`[terse] safeWriteFlag: symlink target ${realFlagDir} is not a directory\n`);
          return;
        }
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[terse] safeWriteFlag: symlink target ${realFlagDir} owned by uid ${realStat.uid}, not current user ${process.getuid()}\n`);
            return;
          }
        } else {
          ...
          if (!normalizedReal.toLowerCase().startsWith(normalizedHome.toLowerCase() + path.sep) &&
            ...
            if (debug) process.stderr.write(`[terse] safeWriteFlag: symlink target ${normalizedReal} is outside home directory ${normalizedHome}\n`);
            return;
          }
        }
      } else {
...
// ($CLAUDE_CONFIG_DIR/.terse-history.jsonl).
//
  ...
  try {
    ...
    try {
      ...
      if (lstat.isSymbolicLink()) {
        ...
        if (typeof process.getuid === 'function') {
          if (realStat.uid !== process.getuid()) {
            if (debug) process.stderr.write(`[terse] appendFlag: symlink target ${realDir} owned by uid ${realStat.uid}\n`);
            return;
          }
        } else {
          const home = os.homedir();
